...
import { LOCALE_CODE_MAP } from '~/modules/i18n'
...
const formatter = computed(() => {
  ...
  // 优先使用组件 props 中的 locale，然后是 i18n 的当前语言，最后回退到浏览器语言
  let locale = propsLocale
  if (!locale) {
    const currentLocale = i18nLocale.value as keyof typeof LOCALE_CODE_MAP
    locale = LOCALE_CODE_MAP[currentLocale] || currentLocale
  }
  if (!locale && !import.meta.env.SSR) {
